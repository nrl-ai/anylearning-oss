...
const RELEASES = [
  {
    ...
    whatsNew: {
      href: "https://github.com/nrl-ai/anylearning-oss/releases",
...
const PreviousVersions = ({ versions, pending }) => (
  <section className="mt-16 border-t border-line pt-10" aria-labelledby="previous-versions-heading">
    ...
    <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground">
      Use an older build when you need to reproduce an existing setup or maintain compatibility. For
      new projects, use the latest version above. Select a version to see its available installers.
    </p>
    ...
    {pending ? (
      ...
      <div className="mt-6 overflow-hidden rounded-lg border border-line bg-surface">
        {versions.map((release) => {
          const downloads = ARCHIVE_PLATFORMS.filter((platform) => release.urls[platform.id]);

          return (
            <details key={release.version} className="group border-b border-line last:border-b-0">
      ...
      <p className="mt-6 text-sm text-muted-foreground">No previous downloads are available yet.</p>
    )}
  </section>
);
    ...
    <div className="mt-8 grid gap-4 md:grid-cols-3">
      {PLATFORMS.map((platform) => (
        <PlatformCard
          ...
          note={platform.note}
          warning={
            platform.id === "macos" && release.version === "0.26.5"
...
const DownloadPage = () => {
  ...
  const currentVersions = new Set(releases.map((release) => release.version).filter(Boolean));
      ...
      <div className="mx-auto w-full max-w-5xl flex-1">
        ...
        <main className="pb-20 pt-16">
          ...
          <p className="mt-5 max-w-2xl text-lg text-muted-foreground">
            Label images and train models on your own machine. The app works offline, so your data
            stays private.
          </p>
