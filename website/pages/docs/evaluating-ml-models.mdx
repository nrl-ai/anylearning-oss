...
```mermaid
flowchart LR
  accTitle: Train, validation and test data flow
  accDescr: Labelled data is split so training fits the model, validation guides settings and a held-out test split produces the final report.
...
```mermaid
flowchart TD
  accTitle: Check a feature for data leakage
  accDescr: Decision tree for excluding values that are unavailable at prediction time or are identifiers and near-unique keys.
