 ...
 *
 * The script launches Chrome through the same dependency-free CDP helper as
 * the screenshot recorder. It is intentionally deterministic:
...
const app = String(args.get("--app") || "http://localhost:3021");
...
async function openLabeller(browser) {
  await browser.call("Page.navigate", {
    url: `${app}/projects/dataset?projectId=${projectId}`,
  });
...
async function record(browser, name, action, posterProgress = 0.72) {
  ...
  fs.copyFileSync(
    path.join(
      directory,
      `${String(Math.min(count - 1, Math.floor(count * posterProgress))).padStart(5, "0")}.jpg`
    ),
    poster
  );
...
async function readDemoAnnotation() {
  ...
  const annotationResponse = await fetch(
    `${app}/api/projects/${projectId}/data_items/${item.id}/get_annotation`
  );
  if (!annotationResponse.ok) {
    throw new Error(`Cannot read demo annotation: ${annotationResponse.status}`);
  }
  return { itemId: item.id, annotation: await annotationResponse.json() };
}

async function writeDemoAnnotation(itemId, annotation) {
  const response = await fetch(
    `${app}/api/projects/${projectId}/data_items/${itemId}/set_annotation`,
    {
      method: "POST",
  ...
  if (!response.ok) throw new Error(`Cannot write demo annotation: ${response.status}`);
}
...
const browser = await launch({ width: 1440, height: 900, scale: 1 });
await browser.setViewport(1440, 900);
let demoState;
try {
  if (!probeOnly) {
    demoState = await readDemoAnnotation();
    await writeDemoAnnotation(demoState.itemId, []);
  }
  ...
  if (probeOnly) {
    ...
    await record(
      ...
      async () => {
        ...
        await waitFor(
          browser,
          `document.querySelector('[role="dialog"][aria-label="Choose a class"]') instanceof HTMLElement`
        );
        await sleep(1500);
        ...
        await waitFor(
          browser,
          `!document.querySelector('[role="dialog"][aria-label="Choose a class"]')`
        );
        await sleep(1200);
  ...
  if (demoState) await writeDemoAnnotation(demoState.itemId, demoState.annotation);
  await browser.close();
}
