 ...
 *
 *   1. start the app in development:  python anylearning/app.py --port 5678 --development
...
const TUTORIALS = ["object-detection"];
...
async function resolveProject(page, wanted) {
  ...
  const match = projects?.find((p) => p.name === wanted.name) ?? projects?.find((p) => p.type === wanted.type);
  if (!match) {
    throw new Error(
