...
const links = [
  ...
  { key: "github", href: "https://github.com/nrl-ai/anylearning-oss", label: "GitHub" },
    ...
    <header className="border-b border-line">
      <div className="mx-auto w-full max-w-6xl px-6">
        <div className="flex h-16 items-center justify-between gap-4">
          <a href="/" aria-label="AnyLearning home" className={`rounded text-foreground ${focusRing}`}>
            <Wordmark />
          </a>
              ...
              {links.map((link) => {
                ...
                return (
                  ...
                  >
                    {link.label}
                    {isActive && (
                      <span aria-hidden="true" className="absolute inset-x-3 -bottom-[13px] h-0.5 rounded-full bg-mark" />
                    )}
                  </a>
                );
              })}
              ...
              <div className="mb-3 flex flex-col overflow-hidden rounded-lg border border-line bg-surface py-1 shadow-lg">
                {links.map((link) => {
                  ...
                  return (
                    ...
                    >
                      {isActive && (
                        <span aria-hidden="true" className="absolute inset-y-1.5 left-0 w-0.5 rounded-full bg-mark" />
                      )}
                      {link.label}
                    </motion.a>
                  );
