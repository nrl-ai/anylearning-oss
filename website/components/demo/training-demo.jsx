...
const lossAt = (i) => clamp(0.94 * Math.exp(-0.185 * i) + 0.052 + 0.013 * Math.sin(i * 2.1), 0, 1);
const accAt = (i) => clamp(0.945 - 0.63 * Math.exp(-0.21 * i) + 0.009 * Math.sin(i * 1.7), 0, 1);
...
function Curve({ label, values, domain, format, ticks }) {
  const [lo, hi] = domain;
  const xAt = (i) => PLOT.left + (i / (TOTAL_EPOCHS - 1)) * (PLOT.right - PLOT.left);
  const yAt = (v) => PLOT.bottom - ((clamp(v, lo, hi) - lo) / (hi - lo)) * (PLOT.bottom - PLOT.top);

  const points = values.map((v, i) => `${xAt(i).toFixed(2)},${yAt(v).toFixed(2)}`);
  const last = values.length - 1;
  const area =
    values.length > 1
      ? `M ${xAt(0).toFixed(2)},${PLOT.bottom} L ${points.join(" L ")} L ${xAt(last).toFixed(
          2
        )},${PLOT.bottom} Z`
      : null;
...
export default function TrainingDemo({ className = "", modelName = "nanodet-plus-m · 320" }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setEpoch((e) => Math.min(e + 1, TOTAL_EPOCHS)), STEP_MS);
        ...
        <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-2">
          ...
          <dl className="flex gap-6">
            ...
            <div>
              ...
              <dd className="font-mono tabular text-sm text-foreground">{elapsed}s</dd>
            </div>
          </dl>
        </div>
