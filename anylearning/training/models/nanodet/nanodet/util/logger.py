...
class NanoDetLightningLogger(LightningLoggerBase):
    ...
    def finalize(self, status):
        ...
        self.save()
        # Release logs.txt. Windows will not delete a file that any handle
        # still holds open, so leaving it attached made the training folder
        # undeletable -- and the cleanup ignores errors, so every run there
        # leaked its working directory silently.
