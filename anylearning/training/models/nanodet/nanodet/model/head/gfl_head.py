...
class Integral(nn.Module):
    ...
    def forward(self, x):
        ...
        # torch.matmul, not F.linear. They are the same operation for a
        # one-dimensional weight and produce bit-identical results on CPU and
        # CUDA -- but Apple's graph compiler lowers the F.linear form into a
        # matmul between the wrong two operands and then rejects its own graph:
        #
        ...
        #   MPSGraphExecutable.mm:1232: failed assertion
        #       `original module failed verification'
        #
        # It has taken the *output* shape as an operand. The assertion aborts
        # the process from inside the autograd engine's own thread -- SIGABRT,
        # and faulthandler prints "<no Python frame>" -- at the first training
        # iteration, so object detection could not run on a Mac's GPU at all.
        # The failing node is LinearBackward0; the forward is fine.
        #
        # Free on the platforms that already worked: measured on an RTX 3080
        # this form is 0.112 ms against F.linear's 0.229 for the shapes this
        # head produces, and identical to the last bit.
...
class GFLHead(nn.Module):
    ...
    def post_process(self, preds, meta):
        ...
        det_results = {}
        warp_matrixes = (
            meta["warp_matrix"] if isinstance(meta["warp_matrix"], list) else meta["warp_matrix"]
        )
        img_heights = (
    ...
    def show_result(self, img, dets, class_names, score_thres=0.3, save_path=None):
