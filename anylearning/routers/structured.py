...
def inspect_huggingface(dataset_id: str) -> dict[str, Any]:
    ...
    normalized = {item.casefold() for item in licenses}
    license_review_required = not bool(normalized & REDISTRIBUTABLE_LICENSES)
    return {
...
def import_huggingface_dataset(project_id: int, body: HuggingFaceImportRequest):
    ...
    if info["license_review_required"] and not body.acknowledge_license:
        raise HTTPException(
            status_code=409,
            detail="This dataset has no recognized permissive license in its card. Review the dataset page and acknowledge its terms before importing.",
        )
    if (
