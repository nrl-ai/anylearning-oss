...
# initialising CUDA. Training is started with multiprocessing.Process, which
# forks on Linux, and a process that has initialised CUDA cannot fork a child
# that uses it -- so the server merely *reporting* which hardware exists would
# otherwise break every GPU run after the first report. Set before torch is
# imported anywhere; it is read at import time.
os.environ.setdefault("PYTORCH_NVML_BASED_CUDA_CHECK", "1")


from anylearning import weights, window_chrome
