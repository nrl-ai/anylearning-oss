import { Circle as Circ, Ellipse as Elp, Polyline, Rect, Text } from "@svgdotjs/svg.js"
import { ArrayXY } from "@svgdotjs/svg.js"

import { PointerLike, ShapeBuilder } from "./ShapeBuilder"
import { Color, IlElementExtra, Point, RoundShape } from "./types"
...
export abstract class RoundBuilder<T extends RoundShape> extends ShapeBuilder<T> {
    ...
    draw_md(e: MouseEvent, addShape: () => void) {
        if (e.buttons === 1 && !e.ctrlKey && !this.origin) {
            ...
            this.captureRelease(() => this.draw_mu(ShapeBuilder.lastPointer, addShape))
        }
    }
    ...
    draw_mm(e: MouseEvent, addShape: () => void): void {
        if (this.origin) {
            ...
            const centre: ArrayXY = [(this.origin.X + e.offsetX) / 2, (this.origin.Y + e.offsetY) / 2]
            const radius = this.calculateRadius([e.offsetX, e.offsetY])
            ;[this.element!, this.element!.shadow].forEach((el) =>
                el.size(2 * radius[0], 2 * radius[1]).move(centre[0] - radius[0], centre[1] - radius[1])
            )
        }
    }

    draw_mu(e: PointerLike, addShape: () => void) {
        // Guard rather than assert: the window-level release can arrive after
        // the gesture has already been closed on the SVG.
        if (!this.origin || !this.element) return
        this.freeRelease()
    ...
    editShape_mm(e: MouseEvent) {
        if (this.dragIndex !== undefined) {
            ...
            const diff = this.calculateDifferent([x, y])
            ...
            elem.shape.centerChanged(
                Util.rotate([this.origin.X + diff[0] / 2, this.origin.Y + diff[1] / 2], oldCenter, phi)
            )
            this.setPoints()
