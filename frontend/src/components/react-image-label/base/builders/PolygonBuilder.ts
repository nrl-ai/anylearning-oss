...
export default class PolygonBuilder extends AngledBuilder<Polygon> {
    ...
    addPoint(event: MouseEvent, addAngledShape: () => void) {
        ...
        if (this.startClicked) {
            ...
            const origin = this.element.shape.points[0]
            ...
            let x = event.offsetX,
                y = event.offsetY
            const radius = this.sd.discRadius + this.sd.width / 150
            if (
                this.element.shape.points.length >= 3 &&
                Math.pow(this.element.shape.points[0][0] - x, 2) + Math.pow(this.element.shape.points[0][1] - y, 2) <
                    Math.pow(radius, 2)
            ) {
            ...
            const _radius = this.element.shape.points.length === 1 ? 5 : 3

            if (this.element.shape.points.length === 1) {
                this.svg.mousemove((event: MouseEvent) => this.newPlg_mm(event))
                const disc = this.drawDisc(x, y, _radius, Color.GreenDisc)
                this.element.discs = [disc]
                disc.mouseover(() => {
                    if (this.element!.shape.points.length > 2) disc.animate().attr({ fill: Color.LightGreenLine })
                })
                disc.mouseout(() => {
    ...
    newPlg_mm(event: MouseEvent) {
        ...
        if (this.element.hasConnector) this.element.shape.points.pop()
        else this.element.hasConnector = true
    ...
    editShape_mm(e: MouseEvent) {
        ...
        if (this.dragIndex !== undefined) {
            ...
            const discRadius = this.sd.discRadius
    ...
    addPointAndClose(event: MouseEvent, addAngledShape: () => void) {
        ...
        this.startClicked = true
        const x = this.element.shape.points[0][0]
        const y = this.element.shape.points[0][1]
        this.element.shape.points.push([x, y])
        this.plotAngledShape()
        this.element.discs[0].attr("class", "")
        this.addPoint({ ...event, offsetX: x, offsetY: y }, addAngledShape)
    ...
    stopDraw(): void {
        if (this.element && this.element.shape.categories.length == 0) {
            this.element.remove()
