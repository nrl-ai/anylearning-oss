...
export default function StructuredWorkspace({ projectId, project }: { projectId: number; project: Project }) {
    ...
    const [hfAcknowledge, setHfAcknowledge] = useState(false)
    ...
    const importHuggingFace = async () => {
        ...
        try {
            await api.post(`/api/projects/${projectId}/structured/huggingface`, {
                ...
                split: hfSplit,
                row_limit: hfRows,
                acknowledge_license: hfAcknowledge,
            })
            setOffset(0)
            setColumnOffset(0)
            await refresh()
                    ...
                    <PanelBody className="space-y-4">
                        ...
                        {hfInfo && (
                            <div className="bg-surface-sunken space-y-4 rounded-lg border p-4">
                                ...
                                {hfInfo.license_review_required && (
                                    <label className="border-run-border bg-run-surface flex items-start gap-2 rounded-md border p-3 text-xs">
                                        <Checkbox
                                            checked={hfAcknowledge}
                                            onCheckedChange={(value) => setHfAcknowledge(value === true)}
                                        />
                                        <span>
                                            I reviewed the dataset card and its terms. AnyLearning did not recognize a
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    ...
                                    <Button
                                        onClick={importHuggingFace}
                                        disabled={hfLoading || (hfInfo.license_review_required && !hfAcknowledge)}
                                    >
