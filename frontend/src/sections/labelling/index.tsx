...
const LabelingScreen: React.FC<LabelingScreenProps> = ({ projectId, subset, onExit }) => {
    ...
    useEffect(() => {
        if (!project?.type || !isReady) return
    ...
    const inferenceShape = async (newShapes: Shape[]) => {
        ...
        try {
            ...
            // axios rejects on a non-2xx, so reaching here means success.
            const result = response.data
            const points = result.result.shapes[0].points.map((p: any) => [p.x, p.y])

            let tmpShape
            if (aiShape === "rectangle") {
