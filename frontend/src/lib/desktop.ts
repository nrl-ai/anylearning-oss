 ...
 * relied on spotting an open dialog by role, and the app keeps one mounted
 * while the licence is unactivated, so the title bar silently stopped dragging
 * on Windows for the whole session. Excluding the rectangle the overlay
 * actually covers says the same thing without guessing.
 */
const NO_DRAG = [
    "[data-window-no-drag]",
