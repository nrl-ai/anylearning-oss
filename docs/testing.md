...
| `smoke_test_build.sh`, `smoke_test_training.py` | the *packaged* binary (below) — not pytest |
...
CfgNode that `load_config` *merges* into, and detectron2 registers datasets in a
...
Each class gets a distinct shape *and* colour so a small model can actually
