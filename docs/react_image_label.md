...
- **mouse wheel**: Zoom. Scales with the wheel delta so a trackpad zooms
  smoothly; a full notch is the same 1.25 / 0.8 it has always been. Ctrl is no
  longer required — this is a canvas, and the wheel has nothing else to do.
- **middle-mouse drag**, or **Ctrl + mouse drag**: Pan
- **Delete key**: Delete Annotations
- **Escape**: Stop editing
- **mouse drag**: Drag/Edit/Rotate Annotations

Gestures end on a window-level `mouseup`, not only on one over the canvas.
Releasing past the edge of the image is routine, and without that the builder
kept its half-finished state and silently rejected every later gesture.

Zoom is applied once per animation frame rather than once per event: `zoom()`
re-plots every shape, and a trackpad emits far more events than frames.
Measured at ~4.5 ms per pass with 12,204 points, so it fits in a 16.7 ms frame
— but the cost is still O(shapes), because zooming rescales stored coordinates
instead of moving a viewport. See `docs/TODO.md` for the transform rewrite that
would make it O(1).
