...
from anylearning.routers.training import (
    training_process_context,
    training_process_ended,
)


def test_training_processes_spawn_without_inheriting_accelerator_state():
    assert training_process_context().get_start_method() == "spawn"
...
def test_the_check_reaps_what_it_finds():
    ...
    assert (
        not psutil.pid_exists(other.pid)
        or psutil.Process(other.pid).status() != psutil.STATUS_ZOMBIE
    )
    assert (
