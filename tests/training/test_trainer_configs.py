...
def build(trainer_class, env, training_params, patch_targets=()):
    ...
    for module in patch_targets:
        env["monkeypatch"].setattr(module, "db_manager", database.db_manager, raising=False)
...
def test_base_trainer_creates_its_folders(trainer_env):
    ...
    trainer = build(
        base_trainer.BaseTrainer, trainer_env, params("resnet18", "lightweight"),
...
def test_pretrained_model_falls_back_to_training_from_scratch(trainer_env, value):
    ...
    trainer = build(
        base_trainer.BaseTrainer, trainer_env, training_params,
        patch_targets=(base_trainer,),
    )
...
def test_nanodet_config_agrees_on_the_class_count(trainer_env, size):
    ...
    trainer = build(
        nanodet_trainer.NanoDetTrainer, trainer_env, params("nanodet", size),
...
def test_nanodet_config_carries_the_training_parameters(trainer_env, size):
    ...
    trainer = build(
        nanodet_trainer.NanoDetTrainer, trainer_env,
...
def test_nanodet_class_names_are_ordered_by_label_id(trainer_env):
    ...
    trainer = build(
        nanodet_trainer.NanoDetTrainer, trainer_env, params("nanodet", "lightweight"),
        patch_targets=(nanodet_trainer,),
    )
...
def test_semseg_config_reserves_a_background_class(trainer_env, architecture):
    ...
    trainer = build(
        semseg_trainer.SemSegTrainer, trainer_env, params(architecture, "medium"),
...
def test_classification_config_matches_the_label_count(trainer_env, architecture):
    ...
    trainer = build(
        classification_trainer.ClassificationTrainer, trainer_env,
...
def test_handpose_config_sets_the_output_units(trainer_env, size):
    ...
    trainer = build(
        hp.HandposeClassificationTrainer, trainer_env, params("mlp", size),
        patch_targets=(hp,),
    )
    write_labels(trainer, trainer_env["labels"])
    ...
    assert config["models"]["arch"]["head"]["output_units"] == len(trainer_env["labels"])
