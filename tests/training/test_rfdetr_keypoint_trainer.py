...
def test_legacy_nested_schema_and_pose_shape_are_normalized():
    ...
    names = keypoints.keypoint_names(labels)
    poses = keypoints.instances(
        {
            "data": [
                {
                    "type": "keypoints",
                    "categories": ["locust"],
                    "points": [[10, 12], [20, 18], [30, 25]],
                    "visibility": [2, 1, 0],
                    "bbox": [10, 12, 20, 13],
                }
            ]
    ...
    assert poses == [
        {
            "keypoints": [10.0, 12.0, 2, 20.0, 18.0, 1, 30.0, 25.0, 0],
            "num_keypoints": 2,
            "bbox": [10.0, 12.0, 10.0, 6.0],
        }
    ]


def test_compact_left_right_suffixes_are_paired_for_horizontal_flip():
    from anylearning.training import keypoints

    names = ["head", "anttipL", "anttipR", "forelegL2", "forelegR2"]

    assert keypoints.flip_pairs(names) == [1, 2, 3, 4]
...
@pytest.mark.parametrize(
    ...
    ids=["current-flat-schema", "legacy-per-class-schema"],
)
def test_inference_returns_named_points_and_draws_them(monkeypatch, configured_names):
    import rfdetr
    ...
    config = yaml.safe_dump(
        {
            "data": {
                "keypoint_names": configured_names,
...
def test_dense_inference_draws_landmarks_without_overlapping_names(monkeypatch):
    import rfdetr

    from anylearning.training.trainers import rfdetr_trainer

    names = [f"point_{index}" for index in range(13)]
    predictions = SimpleNamespace(
        xy=np.array(
            [[[8.0 + index * 10, 20.0] for index in range(len(names))]],
            dtype=np.float32,
        ),
        keypoint_confidence=np.ones((1, len(names)), dtype=np.float32),
        detection_confidence=np.array([0.8], dtype=np.float32),
        data={"xyxy": np.array([[2, 3, 140, 32]], dtype=np.float32)},
    )
    ...
    monkeypatch.setattr(
        rfdetr_trainer.cv2,
        "putText",
        lambda *args, **kwargs: pytest.fail("dense schemas should not draw names"),
    )

    config = yaml.safe_dump({"data": {"keypoint_names": names}})
    results, visualisation = rfdetr_trainer.RFDetrKeypointTrainer.run_inference(
        config, "model.pth", np.zeros((50, 150, 3), dtype=np.uint8)
    )

    assert len(results[0]["keypoints"]) == len(names)
    assert visualisation.sum() > 0
