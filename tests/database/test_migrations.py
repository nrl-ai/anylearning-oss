...
def test_a_new_database_gets_the_schema_and_a_stamp(data_root):
    ...
    assert stamped_revision(main) == "0002_project_data_sources"
...
def test_an_existing_database_is_stamped_rather_than_rebuilt(data_root):
    ...
    connection.execute(
        "INSERT INTO projects (name, type) VALUES ('Kept', 'Object Detection')"
    )
    connection.commit()
    connection.close()
...
def test_a_second_run_changes_nothing(data_root):
    ...
    assert manager.run_all_migrations()["failed"] == []
    assert stamped_revision(data_root / "main.db") == "0002_project_data_sources"


def test_a_legacy_0002_database_remains_readable(data_root):
    """The public build recognizes the last pre-open-source revision stamp."""
    from sqlalchemy import create_engine
