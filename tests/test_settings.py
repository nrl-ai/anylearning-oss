...
def test_gpu_gets_more_workers_than_cpu(store):
    ...
    assert store.resolve_num_workers(on_gpu=True) >= store.resolve_num_workers(
        on_gpu=False
    )
...
def test_power_saving_asks_for_less_than_maximum(store, monkeypatch):
    # Hosted CI may expose only one permitted CPU, where every mode correctly
    # resolves to zero. Give this policy-comparison test a deterministic
    # workstation-sized budget; single-CPU behaviour has its own test below.
    monkeypatch.setattr(store, "_cpus", lambda: 8)
    monkeypatch.setattr(store, "_physical_cores", lambda: 4)
    monkeypatch.setattr(store, "_memory_worker_cap", lambda: 8)
...
def test_a_configured_zero_is_treated_as_unset(store, monkeypatch):
