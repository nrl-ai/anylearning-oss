...
def _polygon_for(shape, box):
    ...
    if shape == "circle":
        ...
        return [
            [cx + rx * math.cos(2 * math.pi * i / 12), cy + ry * math.sin(2 * math.pi * i / 12)]
            for i in range(12)
        ]
...
def build_detection_coco(root, classes=None, per_class=6, size=64, seed=0, subsets=SUBSETS):
    ...
    for subset in subsets:
        ...
        for name, (shape, fill) in classes.items():
            for i in range(per_class):
                ...
                images.append(
                    {"id": image_id, "file_name": filename, "width": size, "height": size}
                )
                annotations.append(
                    {
                        "id": ann_id,
...
OXFORD_PET_ANNOTATIONS_URL = "https://thor.robots.ox.ac.uk/~vgg/data/pets/annotations.tar.gz"
...
def build_detection_yolo(root, classes=None, per_class=6, size=64, seed=0, subsets=SUBSETS):
...
def build_handpose_dataset(root, classes=("open", "fist"), per_class=8, seed=0, subsets=SUBSETS):
