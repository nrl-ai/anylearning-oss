...
def test_labelme_handles_mixed_shape_types_in_one_image():
    data = convert_anylearning_to_labelme(
        [
            shape("rectangle", BOX_POINTS),
            shape("polygon", TRIANGLE_POINTS, label="dog"),
        ],
        "a.jpg",
        IMAGE_SIZE,
    )
...
def test_yolo_survives_malformed_labels():
    ...
    assert (
        convert_anylearning_to_yolo([shape("rectangle", BOX_POINTS)], None, IMAGE_SIZE)
        == ""
    )
    assert (
        convert_anylearning_to_yolo([shape("rectangle", BOX_POINTS)], [{}], IMAGE_SIZE)
        == ""
    )
